...
headphones removed. When you type, only the cat in that Emacs window alternates
paw frames and appears to beat on the keyboard. Cats in other windows remain
idle. After typing stops, it returns to an idle frame.
...
By default, `bongo-cat-color-scheme` is `auto`: it uses the white cat with
black table lines on light mode-lines and the black cat with gold table lines
on dark mode-lines. Set a scheme explicitly before enabling the mode to
preserve one appearance across themes.

```elisp
(setq bongo-cat-color-scheme 'auto)  ; automatic (default)
(setq bongo-cat-color-scheme 'white) ; white cat, black lines
(setq bongo-cat-color-scheme 'black) ; black cat, gold lines
```

If changing the scheme in a running Emacs session, refresh cached images:

```elisp
(setq bongo-cat-color-scheme 'auto)
